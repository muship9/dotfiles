...
LuaSnip の更新が以下のエラーで失敗する場合：

```
You have local changes in ~/.local/share/nvim/lazy/LuaSnip
...
```bash
cd ~/.local/share/nvim/lazy/LuaSnip && git restore .
```

その後 `:Lazy update LuaSnip` で更新できる。
