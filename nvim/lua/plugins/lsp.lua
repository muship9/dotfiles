  ...
  -- TypeScript Tools - Alternative to typescript-language-server
  {
    ...
    config = function()
      ...
      local function find_tsserver_lib(startpath)
        ...
        if volta ~= "" then
          local globbed = vim.fn.glob(volta .. "/tools/image/node/*/lib/node_modules/typescript/lib/tsserverlibrary.js",
            true, true)
          if #globbed > 0 then
      ...
      vim.api.nvim_create_user_command("TSStatus", function()
        vim.notify(("Node: %s\nTypeScript lib: %s"):format(has_node and "OK" or "NG", ts_lib or "not found"),
          vim.log.levels.INFO)
      end, {})
