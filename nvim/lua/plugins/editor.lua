...
return {
	...
	-- Lazygit integration
	{
		...
		config = function()
			...
			-- Fallback to system nvr if available
			if not nvr_binary then
			...
			vim.g.lazygit_use_neovim_remote = nvr_binary and 1 or 0 -- Only use neovim remote if nvr is available
			...
			vim.api.nvim_create_autocmd("TermOpen", {
				...
				callback = function()
					...
					if servername and servername ~= "" and nvr_binary then
						-- Set up neovim-remote environment
						vim.fn.setenv("NVIM", servername)
						local nvr_cmd = nvr_binary .. " --remote-tab-wait +'set bufhidden=wipe'"
						vim.fn.setenv("GIT_EDITOR", nvr_cmd)
						vim.fn.setenv("EDITOR", nvr_cmd)
						vim.fn.setenv("VISUAL", nvr_cmd)
					elseif not nvr_binary then
						-- Fallback to regular nvim if nvr is not available
						vim.fn.setenv("GIT_EDITOR", "nvim")
						vim.fn.setenv("EDITOR", "nvim")
						vim.fn.setenv("VISUAL", "nvim")
						vim.notify("nvr not found, using regular nvim for git operations", vim.log.levels.WARN)
					end
				end,
			})
		end,
